...
document.addEventListener('DOMContentLoaded', () => {
    if (!localStorage.getItem('sessionToken')) {
        document.body.innerHTML = `
            <div style="text-align:center; padding:50px; color:white; font-family:'Prompt';">
                <h2><i class="fa-solid fa-triangle-exclamation"></i> เซสชันไม่ถูกต้อง</h2><br>
                <a href="login.html" style="background:#10b981; color:white; padding:10px 20px; text-decoration:none; border-radius:50px;">ล็อกอินใหม่</a>
            </div>`;
        return;
    }
...
function injectFlatpickr() {
    ...
    script.onload = () => {
        ...
        langScript.src = 'https://cdn.jsdelivr.net/npm/flatpickr/dist/l10n/th.js';
...
function initCustomSelects() {
    ...
    sheet.innerHTML = `
        <div class="cs-header">
            <h3 id="cs-title">กรุณาเลือก</h3>
            <i class="fa-solid fa-xmark cs-close" id="cs-close"></i>
        </div>
        <div class="cs-options" id="cs-options"></div>
    `;
    ...
    document.querySelectorAll('.form-group select').forEach(select => {
        ...
        const updateText = () => {
            let text = select.options.length > 0 && select.selectedIndex >= 0 
        ...
        fakeInput.addEventListener('click', (e) => {
            ...
            if (labelEl) {
                titleText = labelEl.innerHTML.replace(/\<span.*\<\/span\>/g, '').replace(/\<i.*\<\/i\>/g, '').trim();
            }
            document.getElementById('cs-title').innerText = titleText;
            
            const optionsContainer = document.getElementById('cs-options');
            ...
            Array.from(select.options).forEach(opt => {
                ...
                optDiv.innerHTML = `
                    <span>${opt.text}</span>
                    ${select.value === opt.value ? '<i class="fa-solid fa-circle-check"></i>' : ''}
                `;
                optDiv.onclick = () => {
    ...
    try {
        ...
        if (res.success) {
            ...
            if (userDetails.nickname) {
                displayName = `${userDetails.fullName} (${userDetails.nickname})`;
            }
            ...
            document.body.innerHTML = `
                <div style="text-align:center; padding:50px; color:white; font-family:'Prompt';">
                    <h2><i class="fa-solid fa-triangle-exclamation"></i> ${res.message}</h2><br>
                    <a href="login.html" style="background:#10b981; color:white; padding:10px 20px; text-decoration:none; border-radius:50px;">กลับไปหน้าล็อกอิน</a>
                </div>`;
        }
    } catch (e) {
        localStorage.removeItem('sessionToken');
...
function createModalOnTheFly() {
        ...
        <div id="user-history-detail-modal" class="modal">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div style="display:flex; border-bottom:1px solid rgba(255,255,255,0.05); padding:10px 0;">
                        <div style="width:110px; font-weight:bold; color:#93c5fd;">ID:</div>
                        <div id="uhd-id" style="color:#f1f5f9;"></div>
                    </div>
                    <div style="display:flex; border-bottom:1px solid rgba(255,255,255,0.05); padding:10px 0;">
                        <div style="width:110px; font-weight:bold; color:#93c5fd;">ประเภท:</div>
                        <div id="uhd-type" style="color:#f1f5f9;"></div>
                    </div>
                    <div style="display:flex; border-bottom:1px solid rgba(255,255,255,0.05); padding:10px 0;">
                        <div style="width:110px; font-weight:bold; color:#93c5fd;">สถานที่:</div>
                        <div id="uhd-location" style="color:#f1f5f9;"></div>
                    </div>
                    <div style="display:flex; border-bottom:1px solid rgba(255,255,255,0.05); padding:10px 0;">
                        <div style="width:110px; font-weight:bold; color:#93c5fd;">ระยะเวลา:</div>
                        <div id="uhd-date" style="color:#f1f5f9;"></div>
                    </div>
                    <div style="display:flex; border-bottom:1px solid rgba(255,255,255,0.05); padding:10px 0;">
                        <div style="width:110px; font-weight:bold; color:#93c5fd;">จำนวนวัน:</div>
                        <div style="color:#f1f5f9;">
                            <span id="uhd-days" style="font-weight:bold; color:#10b981;"></span>
                            <span id="uhd-days-detail" style="font-size:0.85em; color:#94a3b8; margin-left:5px;"></span>
                        </div>
                    </div>
                    ...
                    <div id="uhd-holiday-zone" style="display:none; background:rgba(239, 68, 68, 0.1); color:#fca5a5; padding:10px; border-radius:8px; border:1px solid rgba(239, 68, 68, 0.2); margin-top:10px;">
                        <div style="display:flex; justify-content:space-between; font-weight:bold; margin-bottom:5px;">
                            <span>วันหยุดรวม:</span><span id="uhd-hol-count"></span>
                        </div>
                        <div style="font-size:0.9em; color:#fca5a5;">
                            <span>รายการ: </span><span id="uhd-hol-list"></span>
                        </div>
                    </div>

                    <div style="display:flex; border-bottom:1px solid rgba(255,255,255,0.05); padding:10px 0;">
                        <div style="width:110px; font-weight:bold; color:#93c5fd;">สถานะ:</div>
                        <div id="uhd-status" style="font-weight:bold;"></div>
                    </div>
                    <div style="margin-top:20px;">
                        <div style="font-weight:bold; color:#93c5fd; margin-bottom:8px;">เหตุผลการลา:</div>
                        <div id="uhd-reason" style="background:rgba(0,0,0,0.2); padding:12px; border-radius:8px; border:1px solid rgba(255,255,255,0.05); white-space:pre-wrap; color:#cbd5e1;"></div>
                    </div>
                </div>
                <div class="modal-footer">
                    <button id="btn-close-detail" class="secondary-btn">ปิด</button>
                </div>
            </div>
        </div>`;
    ...
    document.querySelector('.close-history-btn').onclick = () => {
        document.getElementById('user-history-detail-modal').style.display = 'none';
    };
...
function openUserHistoryDetail(id) {
    const item = myHistoryData.find(x => x.requestID === id);
    if (!item) return;
    ...
    const statusEl = document.getElementById('uhd-status');
...
function updateLeaveOptions(gender) {
    ...
    const specs = ["ลาบวช", "ลาช่วยภรรยาคลอดบุตร", "ลาคลอด"];
    
    for (let i = sel.options.length - 1; i >= 0; i--) {
        if (specs.includes(sel.options[i].value)) sel.remove(i);
    }
...
function calculateDays() {
    ...
    while (curr <= end) {
        if (curr.getDay() !== 0 && !window.annualHolidays.includes(curr.toLocaleDateString('en-CA'))) {
            d++;
        }
...
function resetForm() {
    ...
    document.getElementById('leave-type').dispatchEvent(new Event('change'));

    document.getElementById('leave-unit').selectedIndex = 0;
    document.getElementById('leave-unit').dispatchEvent(new Event('change'));
    ...
    document.getElementById('total-leave-days').innerText = '0';

    document.getElementById('user-location').selectedIndex = 0;
    document.getElementById('user-location').dispatchEvent(new Event('change'));
}
...
function getAuditData(start, end, unit, reason, location, type) {
    ...
    if (actualVal === 1 && unit.startsWith("ครึ่งวัน")) {
        actualVal = 0.5;
        calendarDays -= 0.5;
    }

    return {
        working: workingDays.join(', '),
        hols: holsFound,
        leaveVal: actualVal,
        grandTotal: calendarDays,
        type: type,
        unit: unit,
        start: start,
        end: end,
...
async function submitForm() {
    ...
    if (!data.location) {
        Swal.fire('ข้อมูลไม่ครบ', 'กรุณาเลือกสถานที่ปฏิบัติงาน', 'warning');
        return;
    }
    if (!data.startDate || !data.reason || document.getElementById('total-leave-days').innerText == '0') {
        Swal.fire('ข้อมูลไม่ครบ', 'กรุณากรอกวันที่และเหตุผล', 'warning');
        return;
    }
...
function showSummaryPopup(audit) {
    ...
    if (audit.hols.length > 0) {
        holidayHtml = `
            <div style="background:#fff1f2; padding:10px; border-radius:10px; margin-bottom:10px;">
                <div style="display:flex; justify-content:space-between; color:#e11d48; font-weight:bold;">
                    <span>วันหยุดรวม:</span>
                    <span>${audit.grandTotal} วัน</span>
                </div>
                <div style="font-size:11px; color:#e11d48; margin-top:5px; text-align:right;">
                    ${audit.hols.join(', ')}
                </div>
            </div>`;
    }
    ...
    const html = `
        <div style="background:white; color:#333; padding:25px; border: 4px solid #10b981; border-radius:20px; text-align:left; font-family:'Prompt'; position: relative;">
            ...
            <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
                <span style="font-weight:bold; color:#555;">ผู้ลา:</span>
                <span style="font-weight:bold;">${displayName}</span>
            </div>
            <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
                <span style="font-weight:bold; color:#555;">ประเภท:</span>
                <span>${audit.type} (${audit.unit})</span>
            </div>
            <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
                <span style="font-weight:bold; color:#555;">สถานที่:</span>
                <span>${audit.location || '-'}</span>
            </div>
            <div style="display:flex; justify-content:space-between; margin-bottom:8px;">
                <span style="font-weight:bold; color:#555;">ระยะเวลา:</span>
                <span>${audit.start} ถึง ${audit.end}</span>
            </div>
            <div style="display:flex; justify-content:space-between; margin-top:15px; margin-bottom:15px; align-items:center;">
                <span style="font-weight:bold; color:#555;">วันลาจริง:</span>
                <span style="color:#10b981; font-size:24px; font-weight:bold;">${audit.leaveVal} วัน</span>
            </div>
            <div style="font-size:12px; color:#999; margin-bottom:10px;">วันที่หัก: ${audit.working}</div>
            ${holidayHtml}
            <div style="margin-top:10px;">
                <span style="font-weight:bold; color:#555;">เหตุผล:</span>
                <div style="background:#f8fafc; padding:10px; border-radius:8px; border:1px solid #e2e8f0; margin-top:5px; font-size:14px;">${audit.reason}</div>
            </div>
...
function saveCapture(audit) {
    ...
    html2canvas(captureFrame, { scale: 2, useCORS: true, backgroundColor: '#ffffff', width: 375, windowWidth: 375 }).then(canvas => {
        Swal.fire({
            title: 'บันทึกรูปภาพ',
            text: 'แตะค้างที่รูปภาพเพื่อบันทึก',
...
function fillCaptureContent(audit) {
    ...
    if (audit.hols.length > 0) {
        holidayHtml = `
            <div style="background:#fff1f2; padding:10px; border-radius:10px; margin-bottom:10px;">
                <div style="display:flex; justify-content:space-between; color:#e11d48; font-weight:bold;">
                    <span>วันหยุดรวม:</span><span>${audit.grandTotal} วัน</span>
                </div>
                <div style="font-size:11px; color:#e11d48; margin-top:5px;">
                    ${holItems}
                </div>
            </div>`;
    }

    const row = (l, v, c = 'black', s = '14px') => `
        <div style="display:flex; justify-content:space-between; margin-bottom:10px; border-bottom:1px solid #f1f5f9; padding-bottom:5px;">
            ...
            <span style="text-align:right; color:${c}; font-size:${s}; font-weight:bold;">${v}</span>
        </div>`;
    ...
    container.innerHTML =
        ...
        `<div style="margin-top:10px;">
            <span style="font-weight:bold; color:#555;">เหตุผล:</span>
            <div style="background:#f8fafc; padding:10px; border-radius:8px; border:1px solid #e2e8f0; margin-top:5px; font-size:13px;">${audit.reason}</div>
        </div>`;
...
function displayLeaveHistory(hist) {
    ...
    hist.forEach(h => {
        ...
        const reasonDisplay = h.reason ? h.reason : "-"; 
        
        tbody.innerHTML += `
        <tr>
            <td data-label="ประเภท" style="width: 18%; font-weight:600; color:#fff;">
                ${h.leaveType}
            </td>
            <td data-label="เริ่ม" style="width: 14%; white-space: nowrap; color: #94a3b8;">
                ${h.startDate}
            </td>
            <td data-label="สิ้นสุด" style="width: 14%; white-space: nowrap; color: #94a3b8;">
                ${h.endDate}
            </td>
            <td data-label="เหตุผล" style="width: 30%;">
                <span class="reason-truncate" title="${reasonDisplay}">${reasonDisplay}</span>
            </td>
            <td data-label="สถานะ" style="width: 10%; white-space: nowrap;">
                <span class="status-pill status-${h.status}">${h.status}</span>
            </td>
            <td data-label="จัดการ" style="width: 14%; white-space: nowrap;">
                <div class="action-column">${btnView}${btnSave}${btnCancel}</div>
            </td>
        </tr>`;
    });
}
